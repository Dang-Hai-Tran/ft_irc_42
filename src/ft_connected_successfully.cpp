...
/*   Updated: 2023/12/05 12:44:28 by datran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void sign_in(Server &server, Client **client, int id) {
    ...
    (*client) = oldClient;
    delete tmp;
    if (server.m_getListConnection()[0] == oldClient) {
        std::cout << "OK oldClient is inside list connection." << std::endl;
        std::cout << "Number of connections: " << server.m_getListConnection().size() << std::endl;
    }
    if (DEBUG) {
        std::cout << "Address of oldClient: " << (*client) << std::endl;
    }
}

void connected_successfully(Server &server, Client *client) {
    if (DEBUG) {
        std::cout << "Address of newClient: " << client << std::endl;
    }
